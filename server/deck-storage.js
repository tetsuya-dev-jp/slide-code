...
export class DeckStorage {
  ...
  listDeckIds() {
    this.ensureReady();
    return fs
      .readdirSync(this.decksDir, { withFileTypes: true })
  ...
  listQuarantinedDeckIssues(quarantineDir) {
    const quarantineDecksDir =
      typeof quarantineDir === 'string' && quarantineDir.trim()
    ...
    return fs
      .readdirSync(quarantineDecksDir, { withFileTypes: true })
      .filter((entry) => entry.isDirectory())
      .map((entry) => {
      ...
      .sort((a, b) => {
        const dateCompare = String(b.quarantinedAt || '').localeCompare(
          String(a.quarantinedAt || ''),
        );
