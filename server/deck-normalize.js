...
function normalizeSlides(slides, files) {
  ...
  const normalized = source.map((entry, index) => {
    ...
    const requestedFileId = typeof entry?.fileId === 'string' ? entry.fileId : '';
    const requestedFileRef = typeof entry?.fileRef === 'string' ? entry.fileRef : '';
...
function normalizeAssets(assets) {
  ...
  return source
    .map((entry, index) => {
      ...
      const assetPath = makeUniqueRelativePath(entry.path || entry.name, usedPaths, fallbackName);
