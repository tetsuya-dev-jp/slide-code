...
function resolveDeckTerminalCwd(storage, deckId, baseCwd) {
  ...
  try {
    ...
    const requestedCwd = typeof deck.terminal?.cwd === 'string' ? deck.terminal.cwd.trim() : '';
...
export async function startTerminalWsServer(getContext) {
  ...
  wss.on('listening', () => {
    console.log(
      `Terminal server listening on ws://${runtimeConfig.terminal.wsHost}:${runtimeConfig.terminal.wsPort}`,
    );
  });
  wss.on('error', (err) => {
