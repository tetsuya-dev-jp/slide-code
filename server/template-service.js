...
export function saveTemplateFromDeck({ deckStorage, localTemplateStorage, deckId, payload = {} }) {
  const input = payload && typeof payload === 'object' ? payload : {};
  ...
  const templateId = requestedId ? requestedId : templateIdForDeck(deck.id);
  if (
    hasTemplateForDeck(localTemplateStorage, deck.id) ||
    localTemplateStorage.hasDeck(templateId)
  ) {
    throw makeTemplateExistsError();
  }

  const templateTitle =
    requestedTitle || `${normalizeNonEmptyString(deck.title, '無題のデッキ')} テンプレート`;
...
export function removeTemplatesFromDeck({ localTemplateStorage, deckId }) {
