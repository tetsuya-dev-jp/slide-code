...
function migrateLegacyAppPaths(xdgConfigHome, xdgDataHome) {
  ...
  migrateLegacyDirectory(path.join(xdgDataHome, LEGACY_APP_NAME), path.join(xdgDataHome, APP_NAME));
}
...
export function loadRuntimeConfig() {
  ...
  const xdgConfigHome = resolveXdgBaseDir(
    process.env.XDG_CONFIG_HOME,
    path.join(homeDir, '.config'),
    homeDir,
  );
  const xdgDataHome = resolveXdgBaseDir(
  ...
  const configTerminal =
    config.terminal && typeof config.terminal === 'object' ? config.terminal : {};
  ...
  const apiHost = normalizeHost(process.env.API_HOST || configApi.host, DEFAULT_BIND_HOST);
